...
// inhibit=true (panel open) → bars stay visible, timer paused
function useUIVisibility(inhibit: boolean) {
	...
	// reveal is called on user interaction (tap, swipe, etc.)
	const reveal = useCallback(() => {
		setShowUI(true);
		if (!inhibit) scheduleHide();
	}, [inhibit, scheduleHide]);

	// When a panel opens: cancel timer + force show.
	// When it closes: restart hide timer.
	useEffect(() => {
		if (inhibit) {
			clearTimer();
			// Don't call setShowUI here — just cancel the timer.
			// The bars are already visible because reveal() was called on interaction.
		} else {
			scheduleHide();
		}
		return clearTimer;
...
// ── Hook: auto-play ───────────────────────────────────────────────────────────
// Design rules:
//   • No setState() in an effect body — only inside callbacks / setInterval ticks
//   • No ref.current written during render
//   • router.push() only from event handlers or setTimeout (never from setState updater)
function useAutoPlay(
	autoInterval: AutoInterval,
	totalPages: number,
	nextChapterSlug: string | null,
	getCurrentPage: () => number, // stable callback — reads a ref in the parent
	...
	// Stable ref so the interval body never holds stale prop values
	const stable = useRef({
		autoInterval,
		totalPages,
		nextChapterSlug,
	...
	// Update ref every render — safe because this is not during render, it's an
	// assignment to a ref which React explicitly allows at the top of a component/hook.
	// However, to avoid the "ref during render" lint rule we do it inside useEffect:
	useEffect(() => {
		stable.current = {
			autoInterval,
			totalPages,
	...
	// Start / stop the interval when autoPlay or autoInterval changes
	useEffect(() => {
		if (timerRef.current) clearInterval(timerRef.current);

		if (!autoPlay) {
			// Reset display — deferred via setState in the effect cleanup path is fine
			// as long as it's not *synchronous* inside the effect body before return.
			// Using a 0-timeout keeps it out of the synchronous effect execution path.
			const t = setTimeout(() => setCountdown(autoInterval), 0);
			return () => clearTimeout(t);
		}

		// Initialise countdown display (deferred so it's not synchronous in the body)
		const initTimer = setTimeout(() => setCountdown(autoInterval), 0);

		// Mutable counter lives purely in a closure — no ref write during render
		let remaining = autoInterval;

		timerRef.current = setInterval(() => {
			remaining -= 1;
			// Update display from inside the interval callback — allowed
			setCountdown(remaining);

			if (remaining <= 0) {
				const {
					totalPages: total,
					nextChapterSlug: slug,
				...
				if (nextPage < total) {
					...
					// Navigate after current call stack clears — never inside setState
					setTimeout(() => router.push(`/read/${slug}`), 0);
				}
		...
		// Only restart when the on/off toggle or the chosen interval changes
		// eslint-disable-next-line react-hooks/exhaustive-deps
...
// ── Shared icon button using shadcn Button ────────────────────────────────────
function IconBtn({
	onClick,
	disabled,
	label,
...
export default function MangaReader({
	...
	// Page state — the ref is only ever written inside callbacks/effects, never during render
	const [currentPage, setCurrentPage] = useState(0);
	const currentPageRef = useRef(0); // kept in sync inside goTo, never written at render time
	...
	const goTo = useCallback(
		(page: number, silent = false) => {
			...
			// Safe: writing ref inside a callback, not during render
	...
	// Stable getter for useAutoPlay — reads the ref, never causes stale closure issues
	const getCurrentPage = useCallback(() => currentPageRef.current, []);

	const goNext = useCallback(() => {
		if (currentPageRef.current === totalPages - 1) {
			if (nextChapter) router.push(`/read/${nextChapter.slug}`);
		} else {
			goTo(currentPageRef.current + 1);
		}
	}, [totalPages, nextChapter, goTo, router]);

	const goPrev = useCallback(() => {
		if (currentPageRef.current === 0) {
			if (prevChapter) router.push(`/read/${prevChapter.slug}`);
		} else {
			goTo(currentPageRef.current - 1);
		}
	}, [prevChapter, goTo, router]);

	// ── Auto-play ───────────────────────────────────────────────────────────────
	const { autoPlay, setAutoPlay, countdown } = useAutoPlay(
		autoInterval,
		totalPages,
		nextChapter?.slug ?? null,
		getCurrentPage,
		(page) => goTo(page, true), // silent so it doesn't re-trigger UI reveal on every tick
	);
	...
	}, [bookPairIndex, totalPages, nextChapter, router, goTo]);
	...
	}, [bookPairIndex, prevChapter, router, goTo]);
	...
	return (
		<TooltipProvider delayDuration={150}>
			<div
				...
				style={{
					touchAction: readMode === "scroll" ? "pan-y" : "none",
				}}
				onClick={reveal}
			>
				{/* ══ SNAP ══════════════════════════════════════════════════════════ */}
				...
				{/* ── Persistent counters (visible when bars are hidden) ────────── */}
				<div
					className={cn(
				...
				{/* Autoplay countdown pill — always visible while playing */}
				{autoPlay && (
					<div
						className={cn(
					...
					<div className="bg-gradient-to-t from-background/95 via-background/70 to-transparent px-4 pb-6 pb-safe-bottom pt-14">
						<div className="max-w-3xl mx-auto">
							...
							<div className="flex items-center justify-between">
								...
								{/* Auto-play with countdown ring + badge */}
